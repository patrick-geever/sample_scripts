-- Function: metrics.need_indexed_partition_table_hvdf_msg_pv(name, name, name, name, timestamp with time zone, text)

-- DROP FUNCTION metrics.need_indexed_partition_table_hvdf_msg_pv(name, name, name, name, timestamp with time zone, text);

CREATE OR REPLACE FUNCTION metrics.need_indexed_partition_table_hvdf_msg_pv(tg_table_schema name, tg_archive_schema name, tg_table_name name, partitioning_column_name name, needed_partitioning_date timestamp with time zone, partitioning_interval text)
  RETURNS name AS
$BODY$
-- $Header: $
/**
...
--      SET search_path = myschema_partitions, myschema, public;
      ...
      --pg  Add FK constraint here that points to proper sub-partition table of metrics.hvdf_batch_pv
      s := $$
        ALTER TABLE $$ || quoted_needed_table_name || $$ 
        ADD CONSTRAINT hvdf_msg_pv_batch_primary_value_fkey$$ || to_char( partition_beginning_date, '_YYYYMMDD_') || partitioning_interval || $$ FOREIGN KEY
         (batch_id) REFERENCES metrics.hvdf_batch_pv$$ || to_char( partition_beginning_date, '_YYYYMMDD_') || partitioning_interval || $$ (batch_id); $$;
	execute s;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION metrics.need_indexed_partition_table_hvdf_msg_pv(name, name, name, name, timestamp with time zone, text)
  OWNER TO metrics;
GRANT EXECUTE ON FUNCTION metrics.need_indexed_partition_table_hvdf_msg_pv(name, name, name, name, timestamp with time zone, text) TO public;
GRANT EXECUTE ON FUNCTION metrics.need_indexed_partition_table_hvdf_msg_pv(name, name, name, name, timestamp with time zone, text) TO metrics;
GRANT EXECUTE ON FUNCTION metrics.need_indexed_partition_table_hvdf_msg_pv(name, name, name, name, timestamp with time zone, text) TO role_write_schema_metrics;
